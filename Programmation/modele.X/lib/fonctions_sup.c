...
void menu_principale(bool *stateB,bool *stateN,bool *stateR)
{
    int choix_men=0;
    ...
    do{
        if(*stateB==true){choix_men+=1;delay_en_s(0.2);}
        else if(*stateN==true){moyenne=moyenne+1;operation = moyenne%3;delay_en_s(0.2);}
        else if(*stateR==true){moyenne=moyenne-1;operation = moyenne%3;delay_en_s(0.2);}
        else if(operation==0)
        {
            LCDGoto(0,0);                       //Commence a ecrire sur l'ecran a la case 0,0
            LCDWriteStr(">Actions");
